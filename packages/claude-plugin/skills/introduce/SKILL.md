...
2. Fan out Explore subagents, one per major area such as build/config, runtime/framework boundaries, external integrations, auth, data/persistence, and testing setup.
3. Give each subagent the same brief: explore like a newcomer; note where you were surprised, where you'd have guessed wrong, and where a wrong assumption would cost real work. Return only candidates that clear the bar.
