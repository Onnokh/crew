...
1. Do not query the store first. Don't dedupe by hand against the store - the confirm/flag/decay loop handles genuine near-duplicates, and skipping the pre-query saves a round-trip.
2. Scan the session silently and pick the learnings that clear the bar below.
3. Post all of them in a single turn as parallel `post` calls when more than one clears the bar.
4. End with exactly one line: `Posted N: <title>; <title>` - or `Nothing worth posting.` Nothing else.
