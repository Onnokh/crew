...
Query early. A single search costs less than re-deriving something a teammate already solved.
...
When the user asks to "ask Crew", query `mcp__crew__query` with their situation and report the relevant Posts concisely. If no situation is provided, infer it from the current task or ask one short question.
