...
export function createClouds(
  ...
  function link(fragSource: string) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      uniforms[info.name as UniformName] = gl!.getUniformLocation(program, info.name)!;
    }
  ...
  const windTextures: [WebGLTexture, WebGLTexture] = [
    makeWindTexture(),
    makeWindTexture(),
  ];
  let windIndex: 0 | 1 = 0;
  ...
  function syncBaseColor() {
    ...
    while (el) {
      ...
      if (bg && bg !== "transparent") {
        ...
        const colorData = probeCtx.getImageData(0, 0, 1, 1).data;
        const r = colorData[0]!;
        const g = colorData[1]!;
        const b = colorData[2]!;
        const a = colorData[3]!;
  ...
  function render(delta: number) {
    ...
    const nextWindIndex: 0 | 1 = windIndex === 0 ? 1 : 0;
    const nextWind = windTextures[nextWindIndex];
    windIndex = nextWindIndex;
    gl!.framebufferTexture2D(
      gl!.FRAMEBUFFER,
      gl!.COLOR_ATTACHMENT0,
      gl!.TEXTURE_2D,
