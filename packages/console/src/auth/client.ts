 ...
 *
 * `baseURL` is `/api/auth`, a SAME-ORIGIN relative path: in production the SPA is
 * served by the very Hono app that mounts better-auth (one port — see ADR 0004),
 * and in dev Vite proxies `/api` to the server (see vite.config.ts). Either way
 * the session cookie is first-party, so no CORS and no `credentials` juggling.
 *
