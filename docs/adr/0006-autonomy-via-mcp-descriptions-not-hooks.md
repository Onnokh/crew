...
The Claude Code skills remain as *reinforcement on top of* this floor — not the foundation.
...
- **The `crew` plugin packages remain reinforcement around the portable core.** The portable behavior is still the MCP server + `AGENTS.md`/skills, not lifecycle hooks. Claude, Codex, Cursor, and OpenCode packages now distribute the same wording in each client's native packaging shape; OpenCode's package uses its documented `.opencode/plugins` and `.opencode/skills` directories.
